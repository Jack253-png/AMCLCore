...
import com.mcreater.amclcore.nbtlib.common.tags.CompoundTag;
...
import static com.mcreater.amclcore.nbtlib.mca.io.LoadFlags.*;
...
public class Chunk implements Iterable<Section> {
    ...
    private void initReferences(long loadFlags) {
        ...
        if ((loadFlags & ENTITIES) != 0) {
            entities = level.containsKey("Entities") ? level.getListTag("Entities").toListTag(CompoundTag.class) : null;
        }
        if ((loadFlags & TILE_ENTITIES) != 0) {
            tileEntities = level.containsKey("TileEntities") ? level.getListTag("TileEntities").toListTag(CompoundTag.class) : null;
        }
        if ((loadFlags & TILE_TICKS) != 0) {
            tileTicks = level.containsKey("TileTicks") ? level.getListTag("TileTicks").toListTag(CompoundTag.class) : null;
        }
        if ((loadFlags & LIQUID_TICKS) != 0) {
            liquidTicks = level.containsKey("LiquidTicks") ? level.getListTag("LiquidTicks").toListTag(CompoundTag.class) : null;
        }
        if ((loadFlags & LIGHTS) != 0) {
            lights = level.containsKey("Lights") ? ((ListTag<ListTag<?>>) level.getListTag("Lights")) : null;
        }
        if ((loadFlags & LIQUIDS_TO_BE_TICKED) != 0) {
            liquidsToBeTicked = level.containsKey("LiquidsToBeTicked") ? ((ListTag<ListTag<?>>) level.getListTag("LiquidsToBeTicked")) : null;
        }
        if ((loadFlags & TO_BE_TICKED) != 0) {
            toBeTicked = level.containsKey("ToBeTicked") ? ((ListTag<ListTag<?>>) level.getListTag("ToBeTicked")) : null;
        }
        if ((loadFlags & POST_PROCESSING) != 0) {
            postProcessing = level.containsKey("PostProcessing") ? ((ListTag<ListTag<?>>) level.getListTag("PostProcessing")) : null;
        }
        ...
        if ((loadFlags & (BLOCK_LIGHTS | BLOCK_STATES | SKY_LIGHT)) != 0 && level.containsKey("Sections")) {
            for (CompoundTag section : level.getListTag("Sections").toListTag(CompoundTag.class)) {
    ...
    public static Chunk newChunk(int dataVersion) {
        ...
        c.data = new CompoundTag(new HashMap<>());
        c.data.put("Level", new CompoundTag(new HashMap<>()));
